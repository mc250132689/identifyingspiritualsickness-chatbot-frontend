...
function appendMessage(userText, botText){
  ...
  userMsg.className = "user-msg"; userMsg.textContent = userText; container.appendChild(userMsg);

  const botMsg = document.createElement("div");
  botMsg.className = "bot-msg"; botMsg.innerHTML = botText || "🤲 Generating response..."; container.appendChild(botMsg);
...
async function sendMessage(){
  ...
  if(trained){ appendMessage(`🧍: ${userText}`, `🤖: ${formatBotAnswer(trained)}`); return; }

  const botMsgElem = appendMessage(`🧍: ${userText}`, null);
  try{
    const res = await fetch(API_URL,{method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({message:userText})});
    const data = await res.json();
    botMsgElem.innerHTML = `🤖: ${formatBotAnswer(data.response)}`;
  }catch(err){ botMsgElem.innerHTML="🤖: Error connecting to backend."; console.error(err);}
}
